...
Phase 0P MVP and Phases 1–4.1 are complete. The project now enters Phase 5 adaptive execution: formal runtime signals and reassessment boundaries are the next design work.
...
12. Active: define formal runtime evidence and reassessment boundaries for Phase 5 monotonic adaptive execution.
...
Phase 4.1 has no remaining implementation blocker. Public distribution of a real Evidence Pack remains separately blocked by the ADR-013 written-license gate. The migrated local Pack preserves the existing three reviewed records and bindings; expanding it to the full real policy-eligible AA set requires a future private acquisition with an available `AA_API_KEY`, but does not block the implemented refresh/runtime architecture.
