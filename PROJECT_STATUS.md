...
- Closed the bounded Phase 0P A5p carrier: the model menu puts checked `Auto` above manual controls, labels the effective model/effort explicitly inside the Auto status card, updates that selection and its explanation from the Session projection while a task runs, and exits Auto before applying a manual choice. A changed decision carries its preceding route, so the UI rolls only the changed model and/or effort value to the effective route over 1.2 seconds; Auto and the changed targets use DSH business blue, breathe twice, and return to their normal color, including for an effort-only change. The plugin persists each selection after its current user message and before its effective request header, so the chat timeline records the before/after model and effort plus the tier, reason code, and explanation in that same interval, before the resulting assistant response. Initial projection remains static. Focused component, conversation-node, Loader-composition, and keyless assembled-Web tests passed through fork commit `0fb4723e266ff7aad1f046e5a82473fa5e84f2af`.
