...
- Closed the bounded Phase 0P A5p carrier: the model menu puts checked `Auto` above manual controls, labels the effective model/effort explicitly inside the Auto status card, updates that selection and its explanation from the Session projection while a task runs, and exits Auto before applying a manual choice. A later actual provider/model/effort change briefly highlights both the trigger and effective selection, with a textual switch notice; initial projection does not animate. Focused tests, the full GUI suite, keyless assembled-Web golden snapshots, and browser dogfood passed through fork commit `38c49596040814e688da010d4f46aa62ca987471`.
