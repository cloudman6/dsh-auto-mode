...
8. Host-declared `RecoveryCapability` is a required policy input. [ADR-009](decisions/0009-phase-0p-attributable-worktree-loss-bound.md) accepts possible loss only for Attempt-attributable uncommitted changes in a clean isolated task worktree. No experimental tier, including `strong`, may execute mutable work until a versioned Host provider proves that boundary and `externalSideEffects: 'none'`. Any external effect, unknown attribution, Git repository-state mutation, containment failure, or mutation outside the bound terminates the current Auto attempt. Effects outside the bound must be prevented before they occur; detection after an escape is not capability evidence. User intervention can switch to Manual or wait for a new execution-world declaration; confirmation cannot authorize the blocked Experimental Auto dispatch.
...
- A separately accepted provider design freezes the concrete runner/platform, supported operating systems, dependency ownership, durable journal, and exact production capability/tool-entry inventory. Executor-level real-composition tests cover every filesystem, Web, shell/terminal, background, Code Mode, hook, subagent, direct, and alternate caller; uncovered entries are disabled. Negative tests prove that external effects, ambient-credential exposure, dirty-start drift, Git repository-state or helper mutation, unclassified commands, path, symlink, hard-link, or mount escape, process leakage, every mutation outside the ADR-009 bound, and insufficient or non-reconstructable attribution support are prevented before their effect and terminate Experimental Auto before the next model call. Until the concrete execution-world provider passes those tests, no experimental tier—including `strong`—may execute mutable work; a confirmation action cannot reach provider dispatch.
