...
The concrete preview carrier is the fork model-selection UI plus the plugin's optional Session projection and `/auto` command. It provides a one-operation Auto/manual choice, a checked Auto state, the effective model and optional effort, the localized Light/Standard/Deep handling level, the AA or configured Deep fallback basis, the exact AA snapshot when applicable, and the persisted explanation. A changed decision includes its preceding route, allowing the UI to roll only changed model, effort, or level values to the effective selection over 1.2 seconds; Auto and changed targets use DSH business blue, breathe twice, and return to their normal color. The chat timeline records the before/after route and handling level plus the basis, source snapshot, reason code, and explanation immediately after the triggering user message and before the resulting assistant response. The UI states that AA is heuristic evidence rather than project-Benchmark validation; the production carrier remains undecided.

For the Phase 4.1 plugin contract, `mode: auto` accepts an inline `evidencePack` or `evidencePackPath`. A compatible `aa-evidence-pack-manifest/v1` activates runtime Active Catalog compilation. Optional `hostRoutes` remains an explicit allowlist; when omitted, the plugin derives candidates from current DSH discovery. Every candidate is materialized through `resolveCallConfig()`, retains its complete ExecutionFingerprint, and enters the catalog only through an exact non-quarantined EvidenceRouteKey binding. Optional `deepFallback` remains usable only when its materialized identity is in the eligible Host set. Legacy `aa-evidence-catalog/v1` and Phase 0P seeds remain readable migration/replay paths; schema-v1 prototype tiers are mapped only for historical Sessions.

Phase 4.1 verification on pinned fork commit `9c163d4086d6f12e9a2c8f4151358a9e66955ac1` loads a reusable Pack through the real Loader, runs Light/Standard/Deep, escalation, fallback, explicit failure, cold reconstruction, and Manual scenarios, and proves persisted evidence identity plus complete execution identity agree with request headers. All seven Loader scenarios pass. The unchanged client projection contract also passes the six focused route-change and view tests; the new evidence fields are additive audit facts and do not alter the established UI animation or terminology.
...
Phase 3 Task 6 adds plugin-side composition evidence without changing A1 or A2. Against the same pinned commit, 78 project tests cover dynamic route discovery and exact materialization, all three handling levels, one decision reused across tool steps, monotonic escalation, configured fallback, explicit failure before dispatch, Manual non-interference, and a real persisted Session cold reconstruction with the identical route and explanation. Required namespace registration must exist before `resume()` reads the Session; the verified cold path activates the plugin and then resumes programmatically. A declarative root that resumes inside an earlier composition layer before the Auto plugin registers is outside the tested ordering and must not be claimed as supported.

Phase 3 Task 7 updates only the external plugin projection and fork UI carrier. Schema-v2 append validation rejects prototype tiers, while schema-v1 replay remains readable. The full fork GUI suite passed with 3,767 tests and one skip; focused built-browser replay against local Chrome passed Chinese and English snapshots plus model-only, effort-only, combined, and level-only animation scenarios. The full repository Web suite still requires its configured Playwright browser artifact, which was absent from the verification environment.

Phase 3 Task 8 pins fork commit `9c163d4086d6f12e9a2c8f4151358a9e66955ac1`. One optional cross-repository keyless fixture mounts the external plugin into the built Web scaffold and drives Light, Standard, Deep, and Manual through the real browser, agent loop, Session events, and request header. It proves price-first and latency-second selection inside Standard and exact equality among the displayed route and AA snapshot, persisted selection, and effective request configuration. The full fork GUI suite again passed with 3,767 tests and one skip; four focused browser scenarios, build, lint, Host and Client type checks, and all 28 documentation gates passed. No provider credential was present, so no new Phase 3 live-provider result is claimed.
...
| A5p | One verified carrier for Auto/manual and persisted explanations | Phase 1–3 usability | Handling levels, exact route facts, AA/fallback basis, animations, and legacy replay verified on pinned fork | Fork UI plus plugin Session projection and command |
...
3. **Completed:** implement and verify the seams, now carried by fork commit `9c163d4086d6f12e9a2c8f4151358a9e66955ac1`.
