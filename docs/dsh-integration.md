...
The concrete preview carrier is the fork model-selection UI plus the plugin's optional Session projection and `/auto` command. It provides a one-operation Auto/manual choice, a checked Auto state, the effective model and optional effort, the localized Light/Standard/Deep handling level, the AA or configured Deep fallback basis, and the persisted explanation. A changed decision includes its preceding route, allowing the UI to roll only changed model, effort, or level values to the effective selection over 1.2 seconds; Auto and changed targets use DSH business blue, breathe twice, and return to their normal color. The chat timeline records the before/after route and handling level plus the basis, reason code, and explanation immediately after the triggering user message and before the resulting assistant response. The UI states that AA is heuristic evidence rather than project-Benchmark validation; the production carrier remains undecided.
...
3. **Completed:** implement and verify the seams, now carried by fork commit `afc8d47d597dfdb058a0129a8a95403df16664b4`.
