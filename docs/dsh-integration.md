...
For the Phase 3 plugin contract, `mode: auto` requires either an inline `seed` or a `seedPath`. An `aa-evidence-catalog/v1` seed activates the Phase 3 pipeline. Optional `hostRoutes` is an explicit allowlist of proposed request configurations; when omitted, the plugin derives candidates from current DSH discovery. Every candidate is still materialized through `resolveCallConfig()`. Optional `deepFallback` proposes one exact configured fallback and is usable only if that materialized identity remains in the eligible Host set. The plugin does not define whether these configuration values are global, project-local, or Session-local; that carrier scope remains an open product decision. The historical Phase 0P seed shape remains accepted as a schema-v1 compatibility path, and the maintained client maps its prototype tiers only when replaying those legacy events.
